...
//      r.parseAndEval("for(i in 1:length(add_dic)){mergeUserDic(data.frame(add_dic[i], 'ncn'))}");
      ...
      r.parseAndEval("word <- \'C:/Users/student/Documents/GitHub/snack-review-working/sr-frontend/src/word4.png\'");
//      
      r.parseAndEval("png(word)");
//      
      r.parseAndEval("wordcloud(names(wordcount_top),freq=wordcount_top,scale=c(20,1),rot.per=0.1,min.freq=1,random.order=F,random.color=T,colors=brewer.pal(12, 'Paired'))");
