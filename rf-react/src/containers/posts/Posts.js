...
export const Posts = ({ friendId }) => {
  ...
  const handleScroll = e => {
    ...
    var offset =
      Math.max(window.pageYOffset, d.scrollTop, document.body.scrollTop) +
      window.innerHeight;
    ...
    if (
      ...
      latestPosts.current.length === latestQuery.current.offset + pageCount
    ) {
      setQuery({
  ...
  const likeThisPost = v => {
    likePost(friendId || auth.userKey, v.seq).then(post => {
      console.log(v);
      dispatch({
        type: "updatePost",
        post
        ...
        posts.map(v => {
          return (
            <Post
              key={v.seq}
              {...v}
              username={auth.name ? auth.name : auth.email}
              onLikeClick={e => {
                likeThisPost(v);
                e.preventDefault();
              }}
